<?php
  ...
  if(empty($sentenciacomprobar)) {
    try{
      ...
      $pdo->exec("INSERT INTO tbl_reserva(data_reserva, hora_reserva, hora_fi_reserva, id_mesa) VALUES('{$fecha}', '{$horainicio}', '{$horamas2}', '{$id_mesa}')");
      $pdo->commit();
      header("location:../process/reservas.php?Enviar=Enviar&id_sala={$id_sala}&id_mesa={$id_mesa}");
    }catch(Exception $e){
      $pdo->rollBack();
