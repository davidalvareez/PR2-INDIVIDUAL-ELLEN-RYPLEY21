...
<body class="sala">
    ...
    <h1 class="h1_sala">Lista de las mesas</h1>
    <div class="cuadro_mesas">
        ...
        if (isset($_POST['filtrar'])){
            ...
            $mesa=$pdo->prepare("SELECT tbl_mesa.id_mesa, tbl_mesa.capacidad, tbl_mesa.img, tbl_sala.nom_sala FROM tbl_mesa INNER JOIN tbl_sala ON tbl_mesa.id_sala = tbl_sala.id_sala WHERE tbl_sala.nom_sala LIKE '%{$_POST['nombresala']}%'");
            $mesa->execute();
            ...
            $mesa=$pdo->prepare("SELECT tbl_mesa.id_mesa, tbl_mesa.capacidad, tbl_mesa.img, tbl_sala.nom_sala FROM tbl_mesa INNER JOIN tbl_sala ON tbl_mesa.id_sala = tbl_sala.id_sala");
            $mesa->execute();
            $mesas=$mesa->fetchAll(PDO::FETCH_ASSOC);
            if (empty($mesas)) {
